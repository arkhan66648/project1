...
def main():
    ...
    # We check if the directory exists first to avoid crashing
    if os.path.exists(DIRS['teams']):
        available_teams = {f.replace('.webp', ''): f for f in os.listdir(DIRS['teams']) if f.endswith('.webp')}
    else:
        available_teams = {}

    if os.path.exists(DIRS['sports']):
    ...
    for backend_name in unique_teams:
        ...
        elif team_keys: # Only run if we have downloaded logos
